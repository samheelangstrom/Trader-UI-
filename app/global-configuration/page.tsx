...
export default function GlobalConfigurationPage() {
  ...
  // Add state for margin preview in the component
  const [marginPreview, setMarginPreview] = useState({
    baseMargin: 0.05,
    selectionBias: 2.0,
    probabilityBias: 3.0,
    boundaryMargin: 10.0,
    marginTransition: 5.0,
    effectiveMargin: 5.2,
  })
  ...
  // Add a function to calculate and update the margin preview
  const updateMarginPreview = (params = {}) => {
    const config = { ...marginPreview, ...params }

    try {
      // Example calculation with two selections (50/50 chance)
      const rawProbabilities = [0.5, 0.5]
      const baseMargin = config.baseMargin / 100 // Convert from percentage
      const selBias = [1, 1] // Equal selection bias
      const probBias = config.probabilityBias / 10 // Scale down
      const pBoundary = config.boundaryMargin / 100 // Convert from percentage
      const pTransition = config.marginTransition / 100 // Convert from percentage

      const result = computeMargins(rawProbabilities, baseMargin, selBias, probBias, pBoundary, pTransition)

      // Calculate effective margin as the sum of m_final
      const effectiveMargin = result.reduce((sum, item) => sum + item.m_final, 0) * 100

      setMarginPreview({
        ...config,
        effectiveMargin: Number.parseFloat(effectiveMargin.toFixed(1)),
      })
    } catch (error) {
      console.error("Error calculating margin preview:", error)
    }
  }

  // Update the margin preview when the modal is opened
  useEffect(() => {
    if (showAdvancedMarginModal) {
      updateMarginPreview()
    }
                    ...
                    <div>
                      ...
                      <div className="flex items-center">
                        <input
                          ...
                          max="10"
                          step="0.1"
                          value={marginPreview.baseMargin}
                          onChange={(e) => updateMarginPreview({ baseMargin: Number.parseFloat(e.target.value) })}
                          className="w-full mr-2"
                        />
                        <span className="font-mono w-12 text-right">{marginPreview.baseMargin.toFixed(1)}%</span>
                      </div>
                    ...
                    <div>
                      ...
                      <div className="flex items-center">
                        <input
                          ...
                          max="10"
                          step="0.1"
                          value={marginPreview.selectionBias}
                          onChange={(e) => updateMarginPreview({ selectionBias: Number.parseFloat(e.target.value) })}
                          className="w-full mr-2"
                        />
                        <span className="font-mono w-12 text-right">{marginPreview.selectionBias.toFixed(1)}</span>
                      </div>
                    ...
                    <div>
                      ...
                      <div className="flex items-center">
                        <input
                          ...
                          max="10"
                          step="0.1"
                          value={marginPreview.probabilityBias}
                          onChange={(e) => updateMarginPreview({ probabilityBias: Number.parseFloat(e.target.value) })}
                          className="w-full mr-2"
                        />
                        <span className="font-mono w-12 text-right">{marginPreview.probabilityBias.toFixed(1)}</span>
                      </div>
                      <p className="text-xs text-[#5f6368] mt-1">
                    ...
                    <div>
                      ...
                      <div className="flex items-center">
                        <input
                          ...
                          max="20"
                          step="0.1"
                          value={marginPreview.boundaryMargin}
                          onChange={(e) => updateMarginPreview({ boundaryMargin: Number.parseFloat(e.target.value) })}
                          className="w-full mr-2"
                        />
                        <span className="font-mono w-12 text-right">{marginPreview.boundaryMargin.toFixed(1)}</span>
                      </div>
                      <p className="text-xs text-[#5f6368] mt-1">
                    ...
                    <div>
                      ...
                      <div className="flex items-center">
                        <input
                          ...
                          max="10"
                          step="0.1"
                          value={marginPreview.marginTransition}
                          onChange={(e) => updateMarginPreview({ marginTransition: Number.parseFloat(e.target.value) })}
                          className="w-full mr-2"
                        />
                        <span className="font-mono w-12 text-right">{marginPreview.marginTransition.toFixed(1)}</span>
                      </div>
              ...
              <div className="mb-6">
                ...
                <div className="bg-[#f9f9f9] border border-[#dcdddf] rounded-md p-4 h-48 flex items-center justify-center">
                  <div className="text-center">
                    <div className="text-lg font-medium mb-2">Effective Margin: {marginPreview.effectiveMargin}%</div>
